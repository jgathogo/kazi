...
def run_full_application_package_pipeline(jd_pdf_filename: str, consultant_email: str, timestamp: str) -> bool:
    ...
    # Use the provided timestamp instead of generating a new one
    run_folder_name = f"{base_jd_filename}_{timestamp}"
    settings.log_info(f"Generated timestamped folder for this run: output/job/{run_folder_name}")
    ...
    save_text_to_output_dir(jd_analysis_output_filename, jd_analysis_markdown, base_file=run_folder_name, app_type="job")
    settings.log_info(f"Stage 1: JD Analysis saved to output/job/{run_folder_name}/{jd_analysis_output_filename}")
    ...
    save_text_to_output_dir(tailored_exp_output_filename, tailored_experience_json_str, base_file=run_folder_name, app_type="job")
    settings.log_info(f"Stage 2: Tailored Experience JSON saved to output/job/{run_folder_name}/{tailored_exp_output_filename}")
    ...
    save_text_to_output_dir(narrative_output_filename, strategic_narrative_markdown, base_file=run_folder_name, app_type="job")
    settings.log_info(f"Stage 3: Strategic Narrative saved to output/job/{run_folder_name}/{narrative_output_filename}")
    ...
    save_text_to_output_dir(final_cv_output_filename, final_cv_md, base_file=run_folder_name, app_type="job")
    settings.log_info(f"Stage 4: Final CV Markdown saved to output/job/{run_folder_name}/{final_cv_output_filename}")
    ...
    save_text_to_output_dir(cover_letter_output_filename, cover_letter_md, base_file=run_folder_name, app_type="job")
    settings.log_info(f"Stage 5: Cover Letter Markdown saved to output/job/{run_folder_name}/{cover_letter_output_filename}")
...
def run_jd_analysis_pipeline(jd_pdf_filename: str) -> str | None:
    ...
    saved_filepath = save_text_to_output_dir(output_filename, analysis_markdown, base_file=run_folder_name, app_type="job")
    if saved_filepath:
...
def run_tor_analysis_pipeline(tor_pdf_filename: str, consultant_email: str) -> str | None:
    ...
    save_text_to_output_dir(tor_analysis_output_filename, tor_analysis_json_str, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 2: ToR Analysis JSON saved to output/consultancy/{run_folder_name}/{tor_analysis_output_filename}")
    ...
    save_text_to_output_dir(type_understanding_output_filename, type_and_understanding_md, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 3: Type and Understanding of Assignment sections saved to output/consultancy/{run_folder_name}/{type_understanding_output_filename}")
    ...
    save_text_to_output_dir(framework_output_filename, framework_json_str, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 4: Technical Approach Framework JSON saved to output/consultancy/{run_folder_name}/{framework_output_filename}")
    ...
    save_text_to_output_dir(detailed_tech_approach_output_filename, detailed_technical_approach_md, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 5: Detailed Technical Approach sections saved to output/consultancy/{run_folder_name}/{detailed_tech_approach_output_filename}")
    ...
    save_text_to_output_dir(work_plan_deliverables_output_filename, work_plan_deliverables_md, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 6: Work Plan and Deliverables sections saved to output/consultancy/{run_folder_name}/{work_plan_deliverables_output_filename}")
    ...
    save_text_to_output_dir(general_management_output_filename, general_management_md, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 7: General Management and Compliance sections saved to output/consultancy/{run_folder_name}/{general_management_output_filename}")
    ...
    save_text_to_output_dir(team_experience_output_filename, team_and_experience_md, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 8: Team and Experience sections saved to output/consultancy/{run_folder_name}/{team_experience_output_filename}")
    ...
    save_text_to_output_dir(executive_summary_output_filename, executive_summary_md, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 9: Executive Summary section saved to output/consultancy/{run_folder_name}/{executive_summary_output_filename}")
    ...
    save_text_to_output_dir(full_proposal_output_filename, full_proposal_md, base_file=run_folder_name, app_type="consultancy")
    settings.log_info(f"Stage 10: Full Technical Proposal Markdown saved to output/consultancy/{run_folder_name}/{full_proposal_output_filename}")

    settings.log_info("\n--- ToR Analysis and Technical Proposal Sections Pipeline Completed Successfully ---")
    return full_proposal_md

def select_team_for_proposal() -> dict[str, Any] | None:
    """
    Guides the user through selecting a team (firm or individual consultants) for the proposal.
    Returns a dictionary with selected consultants and firm, or None if the process is aborted.
    """
...
def run_tor_analysis_tailored_pipeline(tor_pdf_filename: str, timestamp: str) -> str | None:
    ...
    # Ensure team_data is a dictionary and access its keys directly
    ...
    settings.log_info(f"Stage 2: Tailored ToR Analysis JSON saved to output/consultancy-tailored/{run_folder_name}/{tor_analysis_output_filename}")
    ...
    settings.log_info(f"Stage 3: Customized Proposal Structure saved to output/consultancy-tailored/{run_folder_name}/{structure_output_filename}")
    ...
    for section in proposal_structure["proposal_structure"]["sections"]:
        ...
        settings.log_info(f"Section {section['section_number']} content saved to output/consultancy-tailored/{run_folder_name}/{section_output_filename}")
    ...
    settings.log_info(f"Stage 5: Customized Proposal saved to output/consultancy-tailored/{run_folder_name}/{full_proposal_output_filename}")
